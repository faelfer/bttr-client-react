module.exports = {
  async skillsFromUserSucessMock(manipulePage, skillsFake) {
    await manipulePage.route("*/**/api/skills/skills_from_user", async (route) => {
      const json = { skills: skillsFake };
      await route.fulfill({ json });
    });
  },
  async skillsByPageSucessMock(manipulePage, skillsFake) {
    await manipulePage.route("*/**/api/skills/skills_by_page?page=1", async (route) => {
      const json = {
  ...
  async skillByIdSucessMock(manipulePage, skillFake) {
    await manipulePage.route(`*/**/api/skills/skill_by_id/${skillFake.id}`, async (route) => {
  ...
  async updateSkillByIdSucessMock(manipulePage, skillIdFake) {
    await manipulePage.route(`*/**/api/skills/update_skill_by_id/${skillIdFake}`, async (route) => {
      const json = {
  ...
  async deleteSkillByIdSucessMock(manipulePage, skillIdFake) {
    await manipulePage.route(`*/**/api/skills/delete_skill_by_id/${skillIdFake}`, async (route) => {
      const json = {
