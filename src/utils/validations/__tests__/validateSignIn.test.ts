...
test("deve retornar valor verdadeiro para o valor vazio do campo email", () => {
  ...
  expect(validateSignIn(emptyEmailInputValues)).toMatchObject(desiredEmptyEmail);
});
...
test("deve retornar valor verdadeiro para o valor vazio do campo senha", () => {
  ...
  expect(validateSignIn(emptyPasswordInputValues)).toMatchObject(desiredInvalidatePassword);
});
